...
export class AuthService {
    ...
    async sendOTP(mobile: string, purpose: string = "LOGIN", email?: string, name?: string): Promise<{ challengeId: string; devOtp?: string }> {
        ...
        if (!emailTo) {
            emailTo = process.env.DEV_OTP_EMAIL;
        }

        // ── Send via Gmail ──────────────────────────────────
        if (emailTo) {
            try {
                await sendOtpEmail(emailTo, otp, Math.round(config.otp.ttlSeconds / 60));
                logger.info(`📧 OTP emailed to ${emailTo} (mobile ${mobile})`);
            } catch (e: any) {
                logger.error(`❌ Failed to email OTP to ${emailTo}: ${e?.message || e}`);
                logger.info(`📱 Fallback — OTP for ${mobile}: ${otp}`);
            }
        } else {
            logger.warn(`⚠️ No email for ${mobile} — OTP only logged below`);
            logger.info(`📱 OTP for ${mobile}: ${otp}`);
        }

        // ── Return challengeId (+ devOtp in non-prod) ───────
        const response: { challengeId: string; devOtp?: string } = {
